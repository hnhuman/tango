...
$(function() {
    ...
    var _myLimitSales = 0;
    var _CanBuy = 0;
    var _checkLimitFun = null ;
    var _IsLogined = false;
    var $cartFun = new $CartComm();
    ...
    var BindEvents = function() {
        ...
        ;
        Base.getScript(skinDomain + "/JS/GoodsDetailPicFun.js?date=160718", function() {
            mycarousel();
            loadShareFun()
        });
                    ...
                    if (userBuyRNOCache["userBuyRnoStr" + _BuyID] != null ) {
                        ...
                        GetJPData("http://api.1yyg.com", "GetUserBuyCodeByBuyid", "codeid=" + _CodeID + "&buyid=" + _BuyID, function(result) {
                            if (result.code == 0) {
                                ...
                                _ShowDialog('<li style="padding-left:65px;">抱歉，查询失败，请刷新页面重试！</li>')
                            }
                        })
                    }
                }
        ...
        var showAllBuyListFun = function() {
            ...
            var getParaFun = function() {
                ...
                _Paras += "&startQueryTime=" + _SendData.startQueryTime;
                _Paras += "&lastTime=" + _SendData.lastTime;
                _Paras += "&lastEIdx=" + _SendData.lastEIdx;
                ...
                var callFun = function(result) {
                    if (result.code == 0) {
                        ...
                        if (CBLRecordFun.TotalCount > 0) {
                            var listItems = result.data;
                            var _html = '<li class="record-menu"><span class="time">时间</span><span class="name">会员</span><span class="people">参与人次</span><span class="ip">IP</span></li>';
                            for (var i = 0; i < listItems.length; i++) {
                                ...
                                _html += '<span class="ip">' + item.ipAddr + " IP:" + item.ip + "</span>";
                                _html += "</li>"
                            }
                            if (_SendData.FIdx == 1 && _SendData.startQueryTime == "") {
                                _SendData.startQueryTime = listItems[0].buyTime
                            }
                            _SendData.lastEIdx = _SendData.EIdx;
                            _SendData.lastTime = listItems[listItems.length - 1].buyTime;
                            ...
                            var _RnoShowObj = $("#divRnoShow");
                            if (_RnoShowObj.length > 0) {
                                _RnoShowObj.hide()
                            }
                ...
                _ulObj.children("li").each(function(idx) {
                    $(this).hover(function(e) {
                        ...
                        _picHover.unbind("click").bind("click", function(e) {
                            ...
                            _index = idx;
                            $("body").find("div.ng-show-bigpic").remove();
                            _img_index_j = _This.attr("indexJ");
                            _img_index_i = _This.attr("indexI");
                            var _htmlImg = '<div class="ng-show-bigpic" style="display:none;" id="infoBox">';
                            if (_img_index_j > 0) {
                                _htmlImg += '<a href="javascript:;" onclick="changeImg(0);" class="z-left-arrow"><i class="z-clump-icon"></i><span></span></a>'
                            } else {
                                _htmlImg += '<a href="javascript:;" onclick="changeImg(0);" class="z-left-arrow" style="display:none;"><i class="z-clump-icon"></i><span></span></a>'
                            }
                            if (_img_index_j < 2) {
                                _htmlImg += '<a href="javascript:;" onclick="changeImg(1);" class="z-right-arrow"><i class="z-clump-icon"></i><span></span></a>'
                            } else {
                                _htmlImg += '<a href="javascript:;" onclick="changeImg(1);" class="z-right-arrow" style="display:none;"><i class="z-clump-icon"></i><span></span></a>'
                            }
                            _htmlImg += '<img  id=\'imgShow\' width="309px"  src="http://postimg.1yyg.com/UserPost/Big/' + _This.attr("pic") + '"  src0="http://postimg.1yyg.com/UserPost/Big/' + _This.attr("pic0") + '"  src1="http://postimg.1yyg.com/UserPost/Big/' + _This.attr("pic1") + '"  src2="http://postimg.1yyg.com/UserPost/Big/' + _This.attr("pic2") + '" />';
                            _htmlImg += "</div>";
                            var _bigPic = $(_htmlImg);
                            $("body").append(_bigPic);
                            ...
                            var _left = (_This.parent().parent().parent()).offset().left;
                    ...
                    if (result.code == 0) {
                        ...
                        if (CBLPageFun.TotalCount > 0) {
                            ...
                            for (var i = 0; i < listItems.length; i++) {
                                ...
                                urlWeb = "http://u.1yyg.com/" + listItems[i].userWeb;
                                urlPhoto = "http://faceimg.1yyg.com/UserFace/" + listItems[i].userPhoto;
                                _html += '<b><a href="' + urlWeb + '" target="_blank" type="showCard" uweb="' + listItems[i].userWeb + '"><img src="' + urlPhoto + '" width="85" height="85"/></a></b>';
                                _html += '<div class="name-line clearfix">';
                                _html += '        <a href="' + urlWeb + '" class="u-name" target="_blank">' + listItems[i].userName + "</a>";
                                _html += "     </div>";
                                _html += '    <p class="mr4"><a><span class="level class-icon' + listItems[i].grade + '"><s></s>' + listItems[i].gradeName + "</span></a></p>";
                                _html += "</div>";
                                _html += '<div class="ng-share-con">';
                                _html += '    <div class="u-data">';
                                urlWeb = "http://post.1yyg.com/detail-" + listItems[i].postID + ".html";
                                _html += '        <div class="u-show"><a href="' + urlWeb + '" target="_blank">第' + listItems[i].codePeriod + "云晒单：" + listItems[i].postTitle + "</a></div>";
                                _html += '        <span class="u-time">' + listItems[i].postTimeEx + "</span>";
                                _html += "    </div>";
                                _html += '    <div class="share-info">';
                                _html += '         <p><a href="' + urlWeb + '" target="_blank">' + listItems[i].postContent.substring(0, 150) + (listItems[i].postContent.length > 150 ? "..." : "") + "</a></p>";
                                _html += "     </div>";
                                _html += '     <div class="pic-list-wrap">';
                                _html += '         <div class="my-list">';
                                _html += '     <ul class="pic-list clearfix" id="ul_' + i + '">';
                                var goodsPic = listItems[i].postAllPic.split(",");
                                var _picLen = goodsPic.length > 3 ? 3 : goodsPic.length;
                                var pic0 = ""
                                  , pic1 = ""
                                  , pic2 = "";
                                if (_picLen == 1) {
                                ...
                                for (var j = 0; j < _picLen; j++) {
                                    urlPhoto = "http://postimg.1yyg.com/UserPost/Small/" + goodsPic[j];
                                    var liid = i + "_" + j;
                                    _html += "<li  pic=" + goodsPic[j] + " indexJ=" + j + " indexI=" + i + " id=" + liid + " pic0=" + pic0 + " pic1=" + pic1 + " pic2=" + pic2 + ">";
                                    _html += '<span><img src="http://skin.1yyg.com/images/loading71.gif" src2=' + urlPhoto + " /></span>";
                                    _html += '<div class="pic-hover" style="display:none;" id="div' + j + '"></div>';
                                    _html += "</li>"
                                }
                                _html += "</ul>";
                                _html += '<div class="talk-wrap">';
                                _html += '<a href="javascript:;"  num="' + listItems[i].postHits + '" postid="' + listItems[i].postID + '" class="xianmu">';
                                _html += "<i></i>羡慕(<em>" + listItems[i].postHits + "</em>)";
                                _html += '<img class="transparent-png" src="http://skin.1yyg.com/post/images/xin.png">';
                                _html += "</a>";
                                urlWeb = "http://post.1yyg.com/detail-" + listItems[i].postID + ".html";
                                _html += '<a href="' + urlWeb + '" target="_blank" class="pinglun">';
                                _html += "<i></i>评论(<em>" + listItems[i].postReplyCount + "</em>)";
                                _html += "</a>";
                                _html += "</div>";
                                _html += "</div>";
                                _html += "</div>";
        ...
        var bindProcessFun = function() {
            ...
            ;
            var showPlFlyFun = function(_cartObj) {
                ...
                if (_IsLoadPlJs) {
                    ...
                    Base.getScript("http://skin.1yyg.com/JS/Parabola.js?date=160518", function() {
                        _IsLoadPlJs = true;
                        showFun()
                    })
                }
            ...
            var add2CartFun = function(_cartObj) {
                ...
                var _Ptn = /^\d+$/;
                if (_Ptn.test(_Num)) {
                    $cartFun.addShopCart(_CodeID, _Num, function(result) {
                        if (result.code == 0) {
                            _IsCartChanged = true;
                            showPlFlyFun(_cartObj)
                        } else {
                            if (result.code == 1) {
                                if (result.num > 0) {
                                    showFailFun("啊哦~！被抢光啦！", function() {
                                        location.reload()
                                    })
                                } else {
                                    showFailFun("啊哦！！剩下的被抢光啦！", function() {
                                        location.reload()
                                    })
                                }
                            } else {
                                if (result.code == 2) {
                                    ...
                                    if (result.code == 3) {
                                        showFailFun("您参与人次已达上限", function() {})
                                    } else {
                ...
                if (_Ptn.test(_Num)) {
                    $cartFun.addShopCart(_CodeID, _Num, function(result) {
                        ...
                        if (_retCode == 0) {
                            ...
                            if (_retCode == 1) {
                                if (result.num > 0) {
                                    showFailFun("啊哦！！不够那么多了！", function() {
                                        location.reload()
                                    })
                                } else {
            ...
            var LoadPageJs = function() {
                var initParamsFun = function() {
                    if (_LimitBuy > 0) {
                        _checkLimitFun = function() {
                            ...
                            GetJPData("http://api.1yyg.com", "checkUserLimitBuy", "codeID=" + _CodeID, function(result) {
                                ...
                                if (result.code == 0) {
                                    ...
                                    var _TxtObj = $("#divNumber").find(".input-num");
                                    if (parseInt(_TxtObj.val()) > _CanBuy) {
                                        _TxtObj.val(_CanBuy)
                                    }
                        ...
                        GetJPData("http://api.1yyg.com", "checklogin", "", function(result) {
                            _IsLogined = result.code == 1;
                            BindEvents()
                        })
                    }
                }
...
var changeImg = function(b) {
    ...
    if (b == 0) {
        _img_index_j--;
        $("#ul_" + _img_index_i + " div").each(function() {
            var f = $(this).attr("id");
            if (f == "div" + _img_index_j) {
                $(this).removeClass("pic-hover").removeClass("pic-hover current").addClass("pic-hover current").show()
            } else {
                $(this).removeClass("pic-hover").removeClass("pic-hover current").addClass("pic-hover").hide()
            }
        });
        if (_img_index_j <= 0) {
            a.attr("src", e);
